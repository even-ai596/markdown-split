...
def save_chunks_to_json(chunks, output_path):
    ...
    chunk_data = [chunk.page_content for chunk in chunks]
    with open(output_path, 'w', encoding='utf-8') as file:
        json.dump(chunk_data, file, ensure_ascii=False, indent=4)

# 主函数
def split_markdown_to_json(input_md_path, output_json_path, use_header_splitter=False, chunk_size=1000, chunk_overlap=100):
    # 读取 .md 文件
    markdown_text = read_markdown_file(input_md_path)
...
split_markdown_to_json(input_md_path, output_json_path, use_header_splitter=True)
